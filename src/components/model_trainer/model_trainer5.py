...
class RecommenderModel:
    ...
    def title_suggestions(self, query: str, limit: int = 8):
        """
        Return up to  display titles that match the query.
        - Case-insensitive
        - Matches against name-only (strips year suffix)
        - Shows "Movie Name (YYYY)" format for display clarity
        Ranked: exact-start matches first, then contains-matches.
        """
        import re
        q = self._normalise(query)
        if not q:
        ...
        for raw in titles:
            ...
            m = re.search(r"-\s*(\d{4})\s*$", raw.strip())
            display = f"{name.title()} ({m.group(1)})" if m else name.title()
